...
[NOTE]
====
Отметим, что объединение поммитов при интерактивном перебазирование -- опция, которой не  обязательно пользоваться.
Однако она может быть удобной, чтобы сгруппировать логически связанные изменения в один коммит.
====
